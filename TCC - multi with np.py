...
try:
    np.savetxt(f"{path}/{modelType}/{numOfParticles}_{concentration}_{cutoff}_teste", posValues)
...
if minimization:
    if __name__ == "__main__":
        ...
        #         hi_y = 0.00
        #         hi_z = 0.00
        #         # posIndexes = np.where((abs(posWithParticles[:,0] - particle[0]) < cutoff*radius) & (abs(posWithParticles[:,1] - particle[1]) < cutoff*radius) & (abs(posWithParticles[:,2] - particle[2]) < cutoff*radius))
        #         posIndexes = np.where((np.sqrt((posWithParticles[:,0] - particle[0])**2+(posWithParticles[:,1] - particle[1])**2+(posWithParticles[:,2] - particle[2])**2) < cutoff*radius))
        #         rows = posIndexes[0]
        #         for a in rows:
        #             print(a)
        #         print(len(rows))
        ...
        def firstThreadUseInMinimization(indexes):
            ...
            for part in range(indexes[0],indexes[1]):
                ...
                hi_z = 0.00
                # posIndexes = np.where((abs(posWithParticles[:,0] - particle[0]) < cutoff*radius) & (abs(posWithParticles[:,1] - particle[1]) < cutoff*radius) & (abs(posWithParticles[:,2] - particle[2]) < cutoff*radius))
                posIndexes = np.where(np.sqrt((posValues[:,0] - particle[0])**2+(posValues[:,1] - particle[1])**2+(posValues[:,2] - particle[2])**2) < cutoff*radius)
                rows = posIndexes[0]
                for part2 in rows:
                    x_2 = posValues[part2,0]
                    y_2 = posValues[part2,1]
        ...
        hysTxt = open(f"{path}/{modelType}/hys_{numOfParticles}_{maxExternalMagField}_{intExchange}_{cutoff}_teste.txt", "w")
        ...
        if hysImgFile:
            ...
            fig.savefig(f"{path}/{modelType}/hys_{numOfParticles}_{maxExternalMagField}_{intExchange:.1f}_{cutoff}_teste.{hysImgFile}",bbox_inches="tight")
