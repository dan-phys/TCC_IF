...
try:
    file = open(f"{path}/{modelType}/{numOfParticles}_{concentration}_ver2.txt", "w")
...
if modelImgFile:
    ...
    plt.savefig(f"{path}/{modelType}/system_{numOfParticles}_{concentration}_ver2.{modelImgFile}",bbox_inches="tight")
    ...
    if __name__ == "__main__":
        ...
        def firstThreadUseInMinimization(indexes):
            ...
            for part in range(indexes[0],indexes[1]):
                ...
                hi_z = 0.00
                particleRange = 3
                for k in range(-particleRange,particleRange+1):
                    z = particle[2] + step*k
                    for j in range(-particleRange,particleRange+1):
                        y = particle[1] + step*j
                        for i in range(-particleRange,particleRange+1):
                            x = particle[0] + step*i
                            testParticle = [x,y,z]
                            if particle != testParticle and testParticle in posWithParticles:
                                indexOfParticle = Particles.index(testParticle)
                                theta_2 = posValues["theta"][indexOfParticle]
                                phi_2 = posValues["phi"][indexOfParticle]
        ...
        hysTxt = open(f"{path}/{modelType}/hys_{numOfParticles}_{concentration}_{maxExternalMagField}_{intExchange}_ver2.txt", "w")
        ...
        if hysImgFile:
            ...
            fig.savefig(f"{path}/{modelType}/hys_{numOfParticles}_{concentration}_{maxExternalMagField}_{intExchange:.1f}_ver2.{hysImgFile}",bbox_inches="tight")
