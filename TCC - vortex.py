...
class MagneticSystem:
    ...
    def hysteresis(savedPositions,maxExternalMagField,th,ph,ImgFile):
        ...
        initialRange = 0
        numOfPools = 9
        for i in range(1,numOfPools + 1):
            finalRange = int(numOfParticles/numOfPools * i)
        ...
        for h in magneticField:
            ...
            if round(h,6) == 0 and mag > 0:
                np.savetxt(f"{savePath}/remanenceData_{maxExternalMagField}_{h:.4f}.txt",posValues[:,:])
            if abs(mag) < abs(minimumCoercivity) and h < 0:
                minimumCoercivity = mag
                np.savetxt(f"{savePath}/coercivityData_{maxExternalMagField}.txt",posValues[:,:])
...
system.hysteresis(numOfParticles,4,np.pi/2,(np.pi*90/180),"pdf")